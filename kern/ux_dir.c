...
ux_diradd(struct inode *dip, const char *name, int inum)
{
        ...
        // 자리 없어서 부모 directory inode에 새로운 data block 할당
...
ux_mkdir(struct inode *dip, struct dentry *dentry, int mode)
{
        ...
        // 이 이름을 갖는 dir가 이미 있는지 확인
        inum = ux_find_entry(dip, (char *)dentry->d_name.name);
        if (inum) {
                return -EEXIST;
        }
        inode = new_inode(sb);  // alloc new in-core inode
        if (!inode) {
                return -ENOSPC;
        }
        inum = ux_ialloc(sb);   // alloc new uxfs inode
        if (!inum) {
                iput(inode);
                return -ENOSPC;
        }
        // 새로 만드는 dir에 대한 inode를 할당했음
...
ux_rmdir(struct inode *dip, struct dentry *dentry)
{
        ...
        // 지우는 directory의 inode number가 들어가야함.
        usb->s_inode[dip->i_ino] = UX_INODE_FREE; // 부모 디렉터리인데?
        usb->s_nifree++;
        return 0;
}
